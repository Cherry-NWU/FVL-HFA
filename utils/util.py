...
import cv2
import sys
sys.path.append('/workspace/DMNet')
...
def get_save_path(args):
    backbone_str = 'vgg' if getattr(args, 'vgg', False) else 'resnet' + str(getattr(args, 'layers', ''))
    base_dir = os.path.join('exp', args.arch, args.data_set, args.arch, 'split{}'.format(args.split), backbone_str)
    args.snapshot_path = os.path.join(base_dir, 'snapshot')
    args.result_path = os.path.join(base_dir, 'result')
