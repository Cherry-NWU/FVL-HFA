...
class Resize(object):
    ...
    def __call__(self, image, label, label2):
        ...
        s_mask2 = cv2.resize(s_mask2.astype(np.float32), dsize=(int(new_w2), int(new_h2)),interpolation=cv2.INTER_NEAREST)
...
class Crop(object):
    ...
    def __call__(self, image, label, label2):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            label2 = cv2.copyMakeBorder(label2, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.ignore_label)
        h, w = label.shape
        raw_label = label
        raw_label2 = label2
        raw_image = image
...
class RandRotate(object):
    ...
    def __call__(self, image, label, label2):
        if random.random() < self.p:
            ...
            label2 = cv2.warpAffine(label2, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=self.ignore_label)
        return image, label, label2
...
class RGB2BGR(object):
    # Converts image from RGB order to BGR order, for model_ori initialized from Caffe
    def __call__(self, image, label, label2):
...
class BGR2RGB(object):
    # Converts image from BGR order to RGB order, for model_ori initialized from Pytorch
    def __call__(self, image, label, label2):
