...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='FVL_HFA')
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--visualize', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='/home/jiayingzi/DMNet/config/iSAID/iSAID_split0_resnet50_DMNet.yaml', help='config file')
    parser.add_argument('--opts', help='see config/ade20k/ade20k_pspnet50.yaml for all options', default=None, nargs=argparse.REMAINDER)
    
    parser.add_argument('--use_esda_fusion', action='store_true', help='Enable ESDA (CLIP-based Zero-Shot) fusion')
    parser.add_argument('--clip_backbone', type=str, default='ViT-L/14')
    parser.add_argument('--text_prompt_type', type=str, default='class_name')
    parser.add_argument('--use_blip2_text', action='store_true', 
                    help='Use BLIP2 generated text instead of class names for ESDA')
    parser.add_argument('--save_blip2_text', action='store_true',
                        help='Save generated BLIP2 texts to files')
    parser.add_argument('--blip2_text_save_path', type=str, default='./blip2_texts_test',
                        help='Path to save BLIP2 generated texts')
...
# ----------------------  DATASET  ----------------------
    ...
    # Val
    if args.evaluate:
        target_val_size = (args.train_h, args.train_w)
        val_transform = transform.Compose([
        transform.Resize(size=target_val_size),
        transform.ToTensor(),
    ...
    # 保存 BLIP2 生成的文本日志           
    if args.save_blip2_text:
        if hasattr(model, 'module'):
            # 如果使用了 DistributedDataParallel
            if hasattr(model.module, 'save_text_log'):
                model.module.save_text_log()
        else:
            # 如果没有使用 DistributedDataParallel
            if hasattr(model, 'save_text_log'):
                model.save_text_log()                  
...
def validate(val_loader, model, val_seed,split):
    ...
    for e in range(db_epoch):
        for i, (input, target,  s_input, s_mask, subcls, ori_label, class_id, image_names) in enumerate(val_loader):
            ...
            # proto_dict =torch.load("/workspace/exp/res50_kshot/split2/res50_12_10_copy/snapshotproto_dict.pth")
            
            # --- 强制 resize 到训练时的尺寸，避免 MoE_Fusion 特征对不齐 ---
            train_h, train_w = args.train_h, args.train_w
            if input.size(2) != train_h or input.size(3) != train_w:
                input = F.interpolate(input, size=(train_h, train_w), mode='bilinear', align_corners=True)
            ...
            if ori_label is not None:
                ori_label = F.interpolate(ori_label.unsqueeze(1).float(),
                                        size=(train_h, train_w),
                                        mode='nearest').squeeze(1).long()
            # -----------------------------------------------------

            
            if args.proto_dict_path is not None and os.path.exists(args.proto_dict_path):
                proto_dict = torch.load(args.proto_dict_path)
            else:
                proto_dict = {}
            ...
            output = model(s_x=s_input, s_y=s_mask, x=input, y=target, classes=subcls, proto_dict=proto_dict,
               query_original_image=input, image_names=image_names)
            # when run baseline
            # output = model(s_x=s_input, s_y=s_mask, x=input, y=target, classes=subcls, proto_dict=proto_dict)
            
            if output is None:
                output = torch.zeros_like(target, dtype=torch.float32)
