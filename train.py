...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/iSAID/iSAID_split0_resnet50_DMNet.yaml',
    ...
    parser.add_argument('--bin_size', type=int, default=32, help='特征池化后的空间尺寸')
    ...
    parser.add_argument('--blip2_text_save_path', type=str, default='./blip2_texts',
...
def train(train_loader, val_loader, model, optimizer, epoch, proto_dict=None,bp_proto_dict=None):
    ...
    for i, (input, target, s_input, s_mask, subcls,image_names) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss, proto_dict, bp_proto_dict = model(s_x=s_input, s_y=s_mask,
                                                              ...
                                                              start_loss=args.start_loss*len(train_loader),
                                                              query_original_image=input,
                                                              image_names=image_names)
        
        # when run Baseline
        # output, main_loss, aux_loss, proto_dict, bp_proto_dict = model(
    ...
    for e in range(db_epoch):
        for i, (input, target, s_input, s_mask, subcls, ori_label, class_id,image_names) in enumerate(val_loader):
            ...
            output = model(s_x=s_input, s_y=s_mask, x=input, y=target, classes=subcls, proto_dict=proto_dict,
               current_iter=current_iter, query_original_image=input,image_names=image_names)
            #when run Baseline
            # output = model(s_x=s_input, s_y=s_mask, x=input, y=target, classes=subcls, proto_dict=proto_dict)
